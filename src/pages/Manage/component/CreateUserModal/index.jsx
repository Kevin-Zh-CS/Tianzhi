...
function CreateModal(props) {
  const { visible, onOk, onCancel, namespace, type } = props;
  ...
  const init = () => {
    Promise.all([getDepartTree(), searchMembers({ ns_id: namespace })]).then(([res1, res2]) => {
 ...
 1.资源库拥有者具有该资源库的所有权限；
 2.首次添加的成员默认具有使用权限，进入成员列表可更改；
 3.资源库拥有者可为其他成员赋予管理权限，赋予其添加用户、移除用户等成员管理权限；
 4.按部门添加的成员拥有一致的权限。`}
          </span>
        }
        showIcon
